...
import fagent.channels.matrix as matrix_module
from fagent.bus.events import OutboundMessage
from fagent.bus.queue import MessageBus
from fagent.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from fagent.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("fagent.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "fagent.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("fagent.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "fagent.channels.matrix.asyncio.create_task", _fake_create_task
    )
