...
![Graph Atlas — визуализация графа знаний](./assets/graph-ui-preview.png)

Что умеет новый UI:

- force-directed layout с живой физической симуляцией
- живая панель поиска с фильтрами по типу узла (kind)
- контекстное меню по правой кнопке мыши на любом узле — раскрывать кластеры, скрывать узлы
- кластерные узлы показывают количество участников и раскрываются inline (участники разлетаются вокруг кластера)
- богатая панель деталей с соседями, рёбрами, метаданными и информацией о кластере
- кнопка показа/скрытия меток (Eye)
- кнопка перезагрузки подтягивает свежие данные с сервера
