...
def _make_provider(config: Config):
    ...
    if provider_name == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            _print_info("Set them in ~/.fagent/config.json under providers.azure_openai section")
            _print_info("Use the model field to specify the deployment name.")
            raise typer.Exit(1)
    ...
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
        ...
        _print_info("Set one in ~/.fagent/config.json under providers section")
        raise typer.Exit(1)
